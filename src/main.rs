...
impl StringType {
    ...
    fn bytes_to_string(&self, data: Vec<u8>) -> Result<String> {
        ...
        match self {
            NO_STRINGS => Err(format_err!("string type was set to NO_STRINGS, but wanted to convert {:#?} to a string", data)),
            ANSI_STR => Ok(String::from_utf8(data)?),
            UNICODE_STR => Ok(String::from_utf16(&data
                    .chunks_exact(2)
                    .into_iter()
...
impl MTFParser {
    ...
    fn parse(&mut self, filename: &str) -> Result<MTFFile> {
        ...
        loop {
            ...
            let dblock = DBLK::parse(&mut cursor, &self.sets)?;
            self.sets.update(dblock.clone());
            ...
            let streams = Stream::parse_all(&mut cursor)?;
            println!("{:#x?}", streams);

            /*
            for stream in streams {
                println!("stream data: {:x?}", stream.read(&mut cursor));
            }
            */
        }
    }
    ...
    fn parse<T: AsRef<[u8]>>(data: &mut Cursor<T>, sets: &DBLKSets) -> Result<DBLK> {
        ...
        let body = match ty {
            DBLKType::TAPE => {
                ...
                let tape_attrs = TapeAttrs::from_bits(tape_attrs).ok_or_else(|| format_err!("could not parse tape attributes from {:#b}", tape_attrs))?;
                let media_sequence_number = data.read_u16::<LittleEndian>()?;
                let password_encryption_algorithm = data.read_u16::<LittleEndian>()?;
                let soft_filemark_block_size = SoftFileMarkBlockSize::parse(data.read_u16::<LittleEndian>()?);
                let media_based_catalog_type = MediaBasedCatalogType::parse(data.read_u16::<LittleEndian>()?)?;
                let media_name = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
                let media_description = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
                let media_password = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
                let software_name = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
            ...
            DBLKType::SSET => {
                ...
                let attrs = SSetAttrs::from_bits(attrs).ok_or_else(|| format_err!("could not parse sset attributes from {:#b}", attrs))?;

                let password_encryption_algorithm = data.read_u16::<LittleEndian>()?;
                let software_compression_algorithm = data.read_u16::<LittleEndian>()?;
                let software_vendor_id = data.read_u16::<LittleEndian>()?;
                let data_set_number = data.read_u16::<LittleEndian>()?;
                let data_set_name = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
                let data_set_description = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
                let data_set_password = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
                let username = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
            ...
            DBLKType::VOLB => {
                let attrs = data.read_u32::<LittleEndian>()?;
                let attrs = VolbAttrs::from_bits(attrs).ok_or_else(|| format_err!("could not parse volb attributes from {:#b}", attrs))?;

                let device_name = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
                let volume_name = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
                let machine_name = TapeAddress::parse(data.read_u32::<LittleEndian>()?, base)?.read_str(&header.string_type, data)?;
            ...
            DBLKType::SFMB => {
                ...
                let mut entries = vec![0u32; ((soft_filemark_block_size.bytes() - 60) / 4) as usize];
...
bitflags! {
    struct CommonBlockAttrsAny: u32 {
...
bitflags! {
    struct CommonBlockAttrsTAPE: u32 {
        const CONTINUATION = 1 << 0;
        const COMPRESSION = 1 << 2;
        const EOS_AT_EOM = 1 << 3;
...
bitflags! {
    struct CommonBlockAttrsSSET: u32 {
        const CONTINUATION = 1 << 0;
        const COMPRESSION = 1 << 2;
        const EOS_AT_EOM = 1 << 3;
...
bitflags! {
    struct CommonBlockAttrsESET: u32 {
        const CONTINUATION = 1 << 0;
        const COMPRESSION = 1 << 2;
        const EOS_AT_EOM = 1 << 3;
...
bitflags! {
    struct CommonBlockAttrsEOTM: u32 {
        const CONTINUATION = 1 << 0;
        const COMPRESSION = 1 << 2;
        const EOS_AT_EOM = 1 << 3;
...
impl TapeAddress {
    ...
    fn read_str<T: AsRef<[u8]>>(self, ty: &StringType, data: &mut Cursor<T>) -> Result<Option<String>> {
        if self.size > 0 {
...
/*
struct ShiftRegister<T> {

}
*/

bitflags! {
    struct FileSystemAttributes: u16 {
...
bitflags! {
    struct MediaFormatAttributes: u16 {
        const CONTINUE = 1 << 0;
        const VARIABLE = 1 << 1;
        const VAR_END = 1 << 2;
        const ENCRYPTED = 1 << 3;
...
impl Stream {
    fn parse<T: AsRef<[u8]>>(data: &mut Cursor<T>) -> Result<Option<Stream>> {
        ...
        let id = String::from_utf8(header_data.read_u32::<LittleEndian>()?.to_le_bytes().to_vec())?;

        // support deprecated mtf's where not all segments have a stream
        match &*id {
            "TAPE" | "SSET" | "VOLB" | "DIRB" | "FILE" | "CFIL" | "ESPB" | "ESET" | "EOTM" | "SFMB" => {
        ...
        let file_system_attributes = FileSystemAttributes::from_bits(file_system_attributes).ok_or_else(|| format_err!("Could not parse stream header file system attributes from {:#b}", file_system_attributes))?;

        let media_format_attributes = header_data.read_u16::<LittleEndian>()?;
        let media_format_attributes = MediaFormatAttributes::from_bits(media_format_attributes).ok_or_else(|| format_err!("Could not parse stream header media format attributes from {:#b}", media_format_attributes))?;
...
fn main() -> Result<()> {
    let f = MTFParser::new().parse("example.bak")?;
